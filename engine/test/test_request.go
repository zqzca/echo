...
const MultipartRequest = `POST /zqzca/echo HTTP/1.1
Host: github.com
Connection: close
...
func RequestTest(t *testing.T, request engine.Request) {
	...
	request.SetURI("/zqzca/echo?token=54321")
	assert.Equal(t, "/zqzca/echo?token=54321", request.URI())

	assert.Equal(t, "/zqzca/echo", request.URL().Path())
